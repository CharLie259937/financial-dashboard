...
def init_db():
    ...
    # 逐股阈值迁移: 旧表补列, 已有股票回填默认值
    cur_cols = [r[1] for r in c.execute("PRAGMA table_info(tracked_stocks)").fetchall()]
    if 'price_threshold' not in cur_cols:
        c.execute("ALTER TABLE tracked_stocks ADD COLUMN price_threshold REAL DEFAULT 5.0")
        c.execute("UPDATE tracked_stocks SET price_threshold=5.0 WHERE price_threshold IS NULL")
    if 'volume_ratio' not in cur_cols:
        c.execute("ALTER TABLE tracked_stocks ADD COLUMN volume_ratio REAL DEFAULT 2.0")
        c.execute("UPDATE tracked_stocks SET volume_ratio=2.0 WHERE volume_ratio IS NULL")
    conn.commit()
    conn.close()
...
def add_tracked_stock(code, market, name="", price_threshold=5.0, volume_ratio=2.0):
    conn = get_db()
    conn.execute(
        "INSERT OR REPLACE INTO tracked_stocks(code, market, name, added_at, price_threshold, volume_ratio) "
        "VALUES(?,?,?,?,?,?)",
        (code.strip().upper(), market, name, datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
         float(price_threshold), float(volume_ratio))
    )
    conn.commit()
    conn.close()
...
def update_stock_thresholds(code, price_threshold, volume_ratio):
    ...
    conn.execute(
        ...
        (float(price_threshold), float(volume_ratio), code)
    )
    conn.commit()
    conn.close()
...
def run_tracking_cycle():
    ...
    for stock in stocks:
        ...
        signals = check_triggers(stock['code'], stock['market'], name, quote, prev,
                                 price_threshold=stock.get('price_threshold'),
    ...
    with tab1:
        ...
        with add_col3:
            new_pt = st.number_input(
                "涨跌幅阈值(%)", min_value=0.1, max_value=20.0, value=5.0, step=0.5,
                key="track_add_pt", help="该股票涨跌幅触发阈值")
        with add_col4:
            new_vr = st.number_input(
                "量比阈值", min_value=1.1, max_value=10.0, value=2.0, step=0.1,
                key="track_add_vr", help="该股票成交量放大触发倍数")
        with add_col5:
            st.write("")
            st.write("")
            if st.button("添加追踪", type="primary", key="track_add_btn"):
                if new_code.strip():
                    add_tracked_stock(new_code, new_market, "", new_pt, new_vr)
        ...
        if not stocks:
            ...
            for stock in stocks:
                ...
                sc1, sc2, sc3, sc4, sc5 = st.columns([2, 1.5, 1, 2, 0.5])
                with sc1:
                ...
                with sc3:
                    if snap:
                        ...
                        st.caption("尚未采集")
                with sc4:
                    st.caption("被动埋点阈值（本股独立）")
                    pt = st.number_input(
                        "涨跌幅阈值(%)", min_value=0.1, max_value=20.0,
                        value=float(stock.get('price_threshold') or 5.0), step=0.5,
                        key=f"pt_{stock['code']}", label_visibility="collapsed")
                    vr = st.number_input(
                        "量比阈值", min_value=1.1, max_value=10.0,
                        value=float(stock.get('volume_ratio') or 2.0), step=0.1,
                        key=f"vr_{stock['code']}", label_visibility="collapsed")
                    if st.button("保存阈值", key=f"save_th_{stock['code']}"):
                        update_stock_thresholds(stock['code'], pt, vr)
                        st.success(f"{stock['code']} 阈值已更新: ±{pt}% / {vr}倍")
                        st.rerun()
                with sc5:
                    st.write("")
